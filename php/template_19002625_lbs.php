...
[e#5 important  = Wake-Adresse#init=20.00.05 ]
[e#6            = Befehls-Pause in ms#init=100 ]
[e#7            = Bus-Sleep-Timeout in Sekunden#init=300 ]
...
E5  : NASA-Adresse für Wake-Up-Paket beim LBS-Start (Default: 20.00.05). Es sollte eine
      Adresse verwendet werden, die nicht existiert.
...
E7  : Bus-Sleep-Timeout in Sekunden (Default: 300, 0 = deaktiviert)
      Wenn für diese Dauer keine Daten vom RS485-Bus empfangen werden, wird A1 auf 1
      gesetzt (Bus schläft). Sobald wieder Daten eintreffen, wird A1 zurück auf 0 gesetzt.
      Nützlich um vor dem ersten Sendebefehl nach einer Pause den Bus explizit zu wecken
      (Wake-Up-Paket via E5).

E8  : Admin-Interface aktivieren (=1) oder deaktivieren (=0)

A1  : Bus-Sleep-Modus (0 = Bus aktiv, 1 = Bus schläft / kein RX seit E7 Sekunden)
...
<br>
<b>v1.01 – 2026-04-12</b><br>
- Bugfix: LongVariable-Werte wurden beim Senden in falscher Byte-Reihenfolge (Little-Endian statt Big-Endian) übertragen – Gerät ignorierte den Befehl<br>
- Bugfix: RX-Parser brach bei 0x34 als Datenbyte vorzeitig ab; Paketerkennung basiert jetzt auf dem Längenfeld im Header<br>
- Bugfix: Rückgabewert von writeArrayToSocket() wird nun geprüft; fehlgeschlagene Sendevorgänge werden geloggt<br>
- Bugfix: Wake-Paket beim LBS-Start wurde vom Waveshare verworfen — 200 ms Pause nach TCP-Connect bis RS485-Seite bereit ist<br>
- Neu: Automatischer Reconnect-Versuch bei TCP-Verbindungsunterbrechung zum Waveshare-Konverter<br>
<br>
<b>v1.00 – 2025-07-05</b><br>
...
$wakeAddr        = trim($E[5]['value']) ?: '20.00.05';
$cmdDelay        = (int)$E[6]['value'] / 1000.0; // ms → Sekunden
$busSleepTimeout = (int)$E[7]['value'];           // Sekunden, 0 = deaktiviert
...
// Bus wecken: kurz warten bis Waveshare RS485-Seite bereit ist, dann Request senden.
// Ohne Pause verwirft der Konverter das erste Paket, da er die RS485-Schnittstelle
// nach dem TCP-Accept erst intern initialisiert.
...
do {
    ...
    if ($binData !== null) {
        ...
        foreach ($binData as $byte) {
            ...
            if ($receiving) {
                ...
                // Nach 3 Bytes: Paketlänge aus Header bestimmen
                // Protokoll: size = Bytes[1]<<8 | Bytes[2]; Gesamtlänge = size + 2
                if ($cnt === 3) {
                    $expectedLen = (($dataBlock[1] << 8) | $dataBlock[2]) + 2;
                    if ($expectedLen < 14 || $expectedLen > 1502) {
                        exec_debug(2, sprintf("RX ungültige Paketlänge %d — verworfen", $expectedLen));
                        $receiving = false;
                        continue;
                    }
                }

                if ($expectedLen === 0 || $cnt < $expectedLen) {
                    continue;
                }

                $receiving = false;
                if ($debugLevel >= 5) exec_debug(5, sprintf("RX Roh-Daten (%d Bytes): %s", $cnt, implode(' ', array_map(fn($b) => sprintf('%02X', $b), $dataBlock))));
    ...
    if (!empty($cmdQueue)) {
        ...
        if ($now - $lastSendTime >= $cmdDelay) {
            ...
            if (isset($cmdQueue[$key])) {
                ...
                if (!$txOk) {
                    exec_debug(0, "TX fehlgeschlagen — versuche Reconnect zu $wsHost:$wsPort ...");
                    if ($waveshare->reconnect()) {
                        exec_debug(1, "Reconnect erfolgreich. Befehl wird erneut gesendet.");
                        $txOk = $waveshare->writeArrayToSocket($encoded);
                        if (!$txOk) {
                            exec_debug(0, "TX nach Reconnect erneut fehlgeschlagen. Befehl verworfen.");
                        }
                    } else {
    ...
    // Bus-Sleep-Erkennung
    if ($busSleepTimeout > 0) {
        $sleeping = (time() - $lastRxTime) >= $busSleepTimeout ? 1 : 0;
        if ($sleeping !== $busSleepState) {
            $busSleepState = $sleeping;
            logic_setOutput($id, 1, $sleeping);
            if ($sleeping) {
                exec_debug(1, "Bus-Sleep erkannt: kein RX seit {$busSleepTimeout}s.");
            } else {
                exec_debug(1, "Bus wieder aktiv: RX empfangen.");
            }
        }
    }
...
function sendWakePacket() {
    ...
    if (!$waveshare->writeArrayToSocket($packet->encode())) {
        exec_debug(0, "Bus wecken: Senden fehlgeschlagen!");
    }
}
